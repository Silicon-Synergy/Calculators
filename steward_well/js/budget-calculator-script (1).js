...
function calculateBudget(
  ...
  // =========================================================================
  // RECOMMENDATION ENGINE — strict processing order:
  //   1. Cashflow  = 10% of Monthly Take-Home Pay (always, unconditionally)
  //   2. REMAINDER = max(0, Monthly Take-Home Pay − Total Expenses)
  //   3. Savings   = 15% of REMAINDER  (0 when REMAINDER ≤ 0)
  //   4. Investment= 20% of REMAINDER  (0 when REMAINDER ≤ 0)
  // =========================================================================

  // Step 1 – Cashflow is always exactly 10 % of take-home pay.
  const recommendedCashflowAmount = 0.10 * monthlyDisposableIncome;

  // Step 2 – Surplus after expenses (never negative).
  const remainder = Math.max(0, monthlyDisposableIncome - totalMonthlyExpenses);

  // Step 3 – Savings: 15 % of remainder.
  const recommendedSavingsAmount  = remainder > 0 ? 0.15 * remainder : 0;

  // Step 4 – Investment: 20 % of remainder.
  const recommendedInvestmentAmount = remainder > 0 ? 0.20 * remainder : 0;

  // Derive percentage-of-REMAINDER for display (used by % input fields).
  // These must be % of remainder so that the round-trip
  // pct → amount → pct stays consistent when the UI converts back.
  const recommendedSavingsPct =
    remainder > 0
      ? (recommendedSavingsAmount  / remainder) * 100
      : 0;
  const recommendedInvestmentsPct =
    remainder > 0
      ? (recommendedInvestmentAmount / remainder) * 100
      : 0;
  // Cashflow pct kept as % of MDI for the CTA / status display only.
  const recommendedCashflowPct =
    monthlyDisposableIncome > 0
      ? (recommendedCashflowAmount / monthlyDisposableIncome) * 100
      : 0;

  const recommendedAllocations = {
    monthly_savings:    recommendedSavingsAmount,
    monthly_investments: recommendedInvestmentAmount,
    monthly_cashflow:   recommendedCashflowAmount,
  };
  // =========================================================================

  // =========================================================================
  // CUSTOM ALLOCATION – honours the user's own % or $ inputs.
  // Source of truth is always REMAINDER (MDI − expenses, floored at 0).
  // If the user's combined amount exceeds REMAINDER we do NOT silently cap —
  // we flag an error so the UI can surface it.
  // =========================================================================
  let customAllocations = null;
  let customAllocationError = null; // will be a string message when breached
  ...
  if (hasUserCustomInput) {
    // Convert user percentages (of REMAINDER) to dollar amounts
    let sAmt = (finalUserSavingsPct / 100) * remainder;
    let iAmt = (finalUserInvestmentsPct / 100) * remainder;
    const combined = sAmt + iAmt;

    // Guard: combined must not exceed REMAINDER
    if (combined > remainder + 0.01) {
      // Flag the error — UI will show it; do NOT mutate the values
      customAllocationError =
        "Your combined Savings & Investment amount exceeds what's left after expenses. Please reduce one or both.";
      // Still produce allocations using the user's raw values so the fields
      // stay populated (the error message prevents acceptance).
      sAmt = Math.max(0, sAmt);
      iAmt = Math.max(0, iAmt);
    }

    sAmt  = Math.floor(sAmt  * 100) / 100;
    iAmt  = Math.floor(iAmt  * 100) / 100;

    // Cashflow = remainder minus what the user allocated to S & I
    let cflow = remainder - (sAmt + iAmt);
    if (cflow < 0 && Math.abs(cflow) < 0.05) cflow = 0; // floating-point dust

    // Recalculate display percentages from final dollar amounts (% of remainder)
    finalUserSavingsPct =
      remainder > 0 ? (sAmt / remainder) * 100 : 0;
    finalUserInvestmentsPct =
      remainder > 0 ? (iAmt / remainder) * 100 : 0;
  ...
  return {
    ...
    remainder: remainder, // surplus after expenses (floored at 0)
    retirement_percentage: retirementPercentage,

    // Recommended amounts & display-only percentages (of MDI)
    ...
    custom_cashflow_pct:
      monthlyDisposableIncome > 0
        ? ((customAllocations ? customAllocations.monthly_cashflow : remainder) /
           monthlyDisposableIncome) * 100
        : 0,
    custom_allocations:      customAllocations,
    custom_allocation_error: customAllocationError, // non-null string when user breached remainder
  };
}
...
function handleExpenseOrAllocationChange() {
  ...
  // 2. Gather custom allocation percentages
...
function updateAllUI(budget) {
  ...
  // Budget zone colours removed — zone field no longer produced by calculateBudget.
  ...
  if (recommendedSavingsPctEl)
    recommendedSavingsPctEl.textContent = budget.remainder > 0 ? "15.0%" : "0.0%";
  if (recommendedInvestmentsPctEl)
    recommendedInvestmentsPctEl.textContent = budget.remainder > 0 ? "20.0%" : "0.0%";
  if (recommendedCashflowPctEl) {
    // Cashflow % = (remainder - savings - investments) / remainder * 100
    // With 15% S + 20% I = 35%, cashflow is 65% of remainder (when remainder > 0)
    const cfPct = budget.remainder > 0
      ? ((budget.remainder - budget.recommended_allocations.monthly_savings - budget.recommended_allocations.monthly_investments) / budget.remainder) * 100
      : 0;
    recommendedCashflowPctEl.textContent = `(${cfPct.toFixed(1)}%)`;
  }
  ...
  // Check if the recommended AMOUNTS have changed (e.g. expenses updated).
  // Compare dollar amounts, not percentages — the percentage fields now show
  // fixed policy rates (15 / 20) so they never change.
  const currentSavingsAmt  = parseFloat(customSavingsAmountInput.value) || 0;
  const currentInvestAmt   = parseFloat(customInvestmentsAmountInput.value) || 0;
  const recSavingsAmt  = budget.recommended_allocations.monthly_savings;
  const recInvestAmt   = budget.recommended_allocations.monthly_investments;

  const isUserEditingSavings    = savingsPercentageInput.matches(":focus") || customSavingsAmountInput.matches(":focus");
  const isUserEditingInvestments = investmentsPercentageInput.matches(":focus") || customInvestmentsAmountInput.matches(":focus");

  const hasRecommendedValuesChanged =
    (!isUserEditingSavings &&
      Math.abs(currentSavingsAmt - recSavingsAmt) > 0.01) ||
    (!isUserEditingInvestments &&
      Math.abs(currentInvestAmt - recInvestAmt) > 0.01);
  ...
  // 3. BUT NOT when user is actively editing
  // 4. AND only when custom toggle is disabled
  ...
  if (shouldAutoPopulate) {
    ...
    if (
      ...
      savingsPercentageInput.value = budget.remainder > 0 ? (15.0).toFixed(1) : (0).toFixed(1);
    }
    if (
      ...
      investmentsPercentageInput.value =
        budget.remainder > 0 ? (20.0).toFixed(1) : (0).toFixed(1);
    }
    if (
      isSavingsAmtEmpty ||
  ...
  // ---- Amount-field sync (only when toggle is OFF and user is not focused) ----
  // When the custom toggle is disabled the amount fields must mirror the
  // engine's recommended amounts directly.  Do NOT re-derive from a percentage
  // value — the percentage shown in the input is % of MDI (for display) while
  // the actual dollar amounts are 15/20 % of REMAINDER.  Re-multiplying a
  // mismatched base was the root cause of the bug.
  ...
  // Cashflow = REMAINDER − (Savings + Investments).  Never negative.
  {
    const rem = budget.remainder; // surplus = MDI − expenses, floored at 0
    const sAmt = budget.custom_allocations
      ? budget.custom_allocations.monthly_savings
      : budget.recommended_allocations.monthly_savings;
    const iAmt = budget.custom_allocations
      ? budget.custom_allocations.monthly_investments
      : budget.recommended_allocations.monthly_investments;

    let cashflowAmount = rem - (sAmt + iAmt);
    if (cashflowAmount < 0 && Math.abs(cashflowAmount) < 0.05) cashflowAmount = 0;
    cashflowAmount = Math.max(0, cashflowAmount);

    const cashflowPct = budget.monthly_disposable_income > 0
      ? (cashflowAmount / budget.monthly_disposable_income) * 100
      : 0;
    ...
    document.getElementById("customCashflowPercentage").textContent =
      `(${cashflowPct.toFixed(1)}%)`;
  }

  // Surface the custom-allocation error (breach of remainder) if present
  {
    const allocErr = document.getElementById("allocationError");
    if (allocErr) {
      if (budget.custom_allocation_error) {
        allocErr.classList.remove("hidden");
        allocErr.textContent = budget.custom_allocation_error;
      } else {
        allocErr.classList.add("hidden");
      }
    }
  }
...
function setupEventListeners() {
  ...
  savingsPercentageInput.addEventListener("input", () => {
    ...
    if (budget && budget.monthly_disposable_income > 0) {
      ...
      const sPct      = parseFloat(savingsPercentageInput.value)    || 0;

      // Convert both to dollar amounts anchored to REMAINDER
      const sAmt  = (sPct  / 100) * rem;
      const iAmt  = (otherPct / 100) * rem;

      const allocErr = document.getElementById("allocationError");

      if (sAmt + iAmt > rem + 0.01) {
        // Show error — do NOT silently cap the value
        if (allocErr) {
          allocErr.classList.remove("hidden");
          allocErr.textContent =
            "Your combined Savings & Investment amount exceeds what's left after expenses. Please reduce one or both.";
        }
        showToast("Savings + Investments cannot exceed the surplus after expenses.");
      } else {
    ...
    if (savingsPercentageInput.value === "0" || savingsPercentageInput.value === "") {
      setTimeout(() => { forceRefreshCharts(); }, 150);
    }
  ...
  investmentsPercentageInput.addEventListener("input", () => {
    ...
    if (budget && budget.monthly_disposable_income > 0) {
      ...
      const iPct      = parseFloat(investmentsPercentageInput.value) || 0;

      const sAmt  = (otherPct / 100) * rem;
      const iAmt  = (iPct     / 100) * rem;
    ...
    if (investmentsPercentageInput.value && investmentsPercentageInput.value !== "0") {
    ...
    if (investmentsPercentageInput.value === "0" || investmentsPercentageInput.value === "") {
      setTimeout(() => { forceRefreshCharts(); }, 150);
    }
  ...
  customSavingsAmountInput.addEventListener("input", (e) => {
    switchToSaveTab();
    if (customSavingsAmountInput.value && customSavingsAmountInput.value !== "") {
      calculatorState.hasCustomSavings = true;
    } else {
      calculatorState.hasCustomSavings = false;
    }
    ...
    if (budget && budget.monthly_disposable_income > 0) {
      const rem      = budget.remainder;
      const otherAmt = parseFloat(customInvestmentsAmountInput.value || "") || 0;
      const sAmt     = parseFloat(customSavingsAmountInput.value  || "") || 0;
      const allocErr = document.getElementById("allocationError");

      if (sAmt + otherAmt > rem + 0.01) {
        if (allocErr) {
          allocErr.classList.remove("hidden");
          allocErr.textContent =
            "Your combined Savings & Investment amount exceeds what's left after expenses. Please reduce one or both.";
  ...
  customInvestmentsAmountInput.addEventListener("input", (e) => {
    switchToSaveTab();
    if (customInvestmentsAmountInput.value && customInvestmentsAmountInput.value !== "") {
      calculatorState.hasCustomInvestments = true;
    } else {
      calculatorState.hasCustomInvestments = false;
    }
    ...
    if (budget2 && budget2.monthly_disposable_income > 0) {
      const rem      = budget2.remainder;
      const otherAmt = parseFloat(customSavingsAmountInput.value || "") || 0;
      const iAmt     = parseFloat(customInvestmentsAmountInput.value || "") || 0;
      const allocErr = document.getElementById("allocationError");

      if (iAmt + otherAmt > rem + 0.01) {
        if (allocErr) {
          allocErr.classList.remove("hidden");
          allocErr.textContent =
            "Your combined Savings & Investment amount exceeds what's left after expenses. Please reduce one or both.";
    ...
    customSavingsToggle.addEventListener("change", () => {
      ...
      if (!isEnabled) {
        ...
        if (budget) {
          ...
          // Set inputs to policy percentages (15% savings, 20% investments of remainder)
          savingsPercentageInput.value =
            budget.remainder > 0 ? (15.0).toFixed(1) : (0).toFixed(1);
          investmentsPercentageInput.value =
            budget.remainder > 0 ? (20.0).toFixed(1) : (0).toFixed(1);
...
function renderCustomProjection(chartType, canvasId, summaryId, titleId) {
  ...
  // Read the live amount fields — these are the single source of truth
  // because they are always validated against REMAINDER.
  const liveSavingsAmt     = parseFloat(customSavingsAmountInput ? customSavingsAmountInput.value : "0") || 0;
  const liveInvestmentsAmt = parseFloat(customInvestmentsAmountInput ? customInvestmentsAmountInput.value : "0") || 0;
  ...
  if (chartType === "savings") {
    if (savingsInputValue === "0" || savingsInputValue === "") {
      ...
      monthlyContribution = liveSavingsAmt;
    }
    returnRate = 3;
    ...
    if (investmentsInputValue === "0" || investmentsInputValue === "") {
      ...
      monthlyContribution = liveInvestmentsAmt;
    }
    returnRate = 10;
  } else {
    // "both"
    const s = (savingsInputValue === "0" || savingsInputValue === "")     ? 0 : liveSavingsAmt;
    const i = (investmentsInputValue === "0" || investmentsInputValue === "") ? 0 : liveInvestmentsAmt;
    monthlyContribution = s + i;
    const total = s + i;
    returnRate = total > 0 ? (s * 3 + i * 10) / total : 6.5;
...
function handleCustomAmountChange(event) {
  ...
  // Calculate the percentage based on the amount relative to REMAINDER
  const budget = calculatorState.currentBudget;
  const rem = budget ? budget.remainder : 0;
  const percentage = rem > 0 ? (value / rem) * 100 : 0;
